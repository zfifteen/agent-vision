...
2. For `snapshot`, create `$HOME/.codex/agent-vision/frames`, choose an absolute output path inside it, run `"$HOME/.codex/plugins/cache/local/agent-vision/1.0.2/dist/agent-vision-capture-file" --output "$OUTPUT" --json`, verify `ok: true`, then display the saved JPEG with a Markdown image link using the absolute path.
